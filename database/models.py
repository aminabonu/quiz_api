...
# sql.execute('alter table users add column username TEXT;')
...
def check_answer(q_id: int, user_id: int, user_answer: str):
    ...
    question_answer = sql.execute('SELECT * FROM questions WHERE question_id=?;', (level, q_set))

    sql.execute('INSERT INTO user_answer VALUES (?,?,?,?);',
                (user_id, q_id, question_asnwer[0], 1 if question_answer[0] == user_answer else 0))
...
# function test end and results
def test_complete_get_position(user_id: int, corrects: int, level: str, q_set: str):
    connection = sqlite3.connect('quiz.db')
    sql = connection.cursor()

    sql.execute('INSWRT INTO user_statistic VALUES (?,?,?,?,?);', (user_id, corrects, level, q_set, datetime.now()))
    connection.commit()
